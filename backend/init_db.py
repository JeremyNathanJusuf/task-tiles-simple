...
from models import (
    User, Board, TaskList, Card, BoardMember, Invitation, CardContributor,
    InvitationStatus, BoardRole
)
from auth import get_password_hash


def init_database():
    """Initialize the database with sample data for collaborative features"""
    ...
    try:
        ...
        demo_progress_list = TaskList(title="In Progress", position=1, board_id=demo_board.id)
        ...
        admin_backlog_list = TaskList(title="Backlog", position=0, board_id=admin_board.id)
        admin_active_list = TaskList(title="Active", position=1, board_id=admin_board.id)
        admin_review_list = TaskList(title="Review", position=2, board_id=admin_board.id)
        admin_complete_list = TaskList(title="Complete", position=3, board_id=admin_board.id)
        
        db.add_all([admin_backlog_list, admin_active_list, admin_review_list, admin_complete_list])
        
        # Lists for Alice's collaborative board
        alice_ideas_list = TaskList(title="Ideas", position=0, board_id=alice_board.id)
        alice_planning_list = TaskList(title="Planning", position=1, board_id=alice_board.id)
        alice_working_list = TaskList(title="Working", position=2, board_id=alice_board.id)
        alice_testing_list = TaskList(title="Testing", position=3, board_id=alice_board.id)
        alice_finished_list = TaskList(title="Finished", position=4, board_id=alice_board.id)
        
        db.add_all([alice_ideas_list, alice_planning_list, alice_working_list, alice_testing_list, alice_finished_list])
        
        # Lists for Bob's development board
        bob_todo_list = TaskList(title="Sprint Backlog", position=0, board_id=bob_board.id)
        bob_progress_list = TaskList(title="In Development", position=1, board_id=bob_board.id)
        ...
        demo_card1 = Card(
            ...
            checklist=["Read the documentation", "Explore the interface", "Create your first board"]
        )
        ...
        demo_card3 = Card(
            ...
            position=0,
            created_by=demo_user.id,
            checklist=["Find the invite button", "Add team member usernames", "Send invitations"]
        )
        ...
        admin_card1 = Card(
            ...
            checklist=["Collect feedback", "Analyze patterns", "Create improvement tickets"]
        )
        ...
        admin_card2 = Card(
            ...
            position=0,
            created_by=admin_user.id,
            checklist=["Review current docs", "Update API documentation", "Add new features"]
        )
        ...
        admin_card3 = Card(
            ...
            position=0,
            created_by=admin_user.id,
            checklist=["Run security scan", "Review access controls", "Update security policies"]
        )
        ...
        alice_card1 = Card(
            ...
            checklist=["Schedule brainstorming session", "Invite all team members", "Document ideas"]
        )
        ...
        alice_card2 = Card(
            ...
            position=0,
            created_by=alice_user.id,
            checklist=["Create wireframes", "Design UI components", "Get team feedback"]
        )
        ...
        alice_card3 = Card(
            ...
            position=0,
            created_by=alice_user.id,
            checklist=["Set up WebSocket connection", "Implement live updates", "Test with multiple users"]
        )
        ...
        alice_card4 = Card(
            ...
            position=0,
            created_by=alice_user.id,
            checklist=["Upload avatar functionality", "Display avatars on cards", "Profile page integration"]
        )
        ...
        bob_card2 = Card(
            ...
            position=0,
            created_by=bob_user.id,
            checklist=["Identify slow endpoints", "Add database indexes", "Implement caching"]
        )
        ...
        db.add_all([contrib8, contrib9, contrib10, contrib11, contrib12, contrib13, contrib14, contrib15, contrib16, contrib17])
